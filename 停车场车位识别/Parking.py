...
class Parking:
    ...
    def identify_blocks(self,image, lines, make_copy=True):
       ...
       # self.cv_show("1",image)

        # image_=self.draw_lines(new_image,lines,(0,0,255),2)
        # self.cv_show("2", image_)
        #Step 1: 过滤部分直线
        cleaned = []
        for line in lines:
            for x1,y1,x2,y2 in line:
        ...
        #Step 2: 对直线按照x1进行排序
        import operator
        list1 = sorted(cleaned, key=operator.itemgetter(0, 1))  #根据第一个点横纵坐标排序
        ...
        for i in range(len(list1) - 1):
            distance = abs(list1[i+1][0] - list1[i][0])    #距离相邻的两条直线横向差
            if distance <= clus_dist:  #将距离小于10的聚为一类
        ...
        for key in clusters:
            ...
            cleaned = list(set(all_list))  #每一类的所有直线
            if len(cleaned) > 5:  #滤除直线数小于5的类
                cleaned = sorted(cleaned, key=lambda tup: tup[1])  #根据第一个点的纵坐标排序
                avg_y1 = cleaned[0][1]
                avg_y2 = cleaned[-1][1]
                avg_x1 = 0
                avg_x2 = 0
                for tup in cleaned:
                    avg_x1 += tup[0]
                    avg_x2 += tup[2]
                avg_x1 = avg_x1/len(cleaned)#取横坐标均值
                avg_x2 = avg_x2/len(cleaned)
                rects[i] = (avg_x1, avg_y1, avg_x2, avg_y2)  #构造矩形
                i += 1
        ...
        buff = 7
        for key in rects:
            tup_topLeft = (int(rects[key][0] - buff), int(rects[key][1]))  #矩形左上角点横坐标左移7个像素
            tup_botRight = (int(rects[key][2] + buff), int(rects[key][3])) #矩形右下角点横坐标右移7个像素
